...
void print_usage(const char* argv0) {
    std::cerr << "Usage:\n"
              ...
              << "  " << argv0 << " detach --iface <ifname>\n"
              << "  " << argv0 << " stats  --iface <ifname>\n";
}
...
Result<void, SLError> cmd_attach(const Options& opts) {
    ...
    // Keep the program attached — in production this would persist via pinning.
    // For the CLI tool we just report success; the BPF program remains loaded
    // in the kernel even after this process exits (kernel holds the fd).
    return Result<void, SLError>::ok();
}
...
Result<void, SLError> cmd_detach(const Options& opts) {
    ...
    int err = bpf_xdp_detach(static_cast<int>(ifindex), 0, nullptr);
    if (err) {
        return Result<void, SLError>::error(
...
Result<void, SLError> cmd_stats(const Options& opts) {
    ...
    // Query XDP link info from the kernel
    struct bpf_xdp_query_opts query_opts = {};
    query_opts.sz = sizeof(query_opts);
    int err = bpf_xdp_query(static_cast<int>(ifindex), 0, &query_opts);
    if (err) {
        return Result<void, SLError>::error(
            SLError{SLErrorCode::IOError,
    ...
    std::cout << "XDP stats for " << opts.ifname << " (ifindex=" << ifindex << "):\n"
              ...
              << "  drv_prog_id: " << query_opts.drv_prog_id << "\n"
              << "  hw_prog_id:  " << query_opts.hw_prog_id << "\n"
              << "  skb_prog_id: " << query_opts.skb_prog_id << "\n";

    if (query_opts.prog_id == 0) {
