    ...
    while (progress) {
        ...
        for (const auto& [inner_id, outer_id] : fold_pairs) {
            ...
            if (outer_res.value()->op == OpType::Transpose) {
                auto o_res = g.get_node(outer_id);
                if (o_res.has_value()) {
                    const Node* inner_check = nullptr;
                    // inner is already removed, so we stored its attrs earlier.
                    // We didn't capture attrs, so for Transpose composition
                    // we note this as a basic fold. A more sophisticated pass
                    // would compose the permutation vectors.
