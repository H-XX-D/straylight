...
Result<VpuSlabAlloc, SLError> SpliceEngine::vpu_alloc_block(int slab_order) {
    ...
    if (vpu_fd_ >= 0) {
        ...
        int rc = ::ioctl(vpu_fd_,
                         ...
                         &req);
        if (rc < 0) {
            return Result<VpuSlabAlloc, SLError>::error(
                SLError{SLErrorCode::HardwareFault,
                        std::string("VPU alloc ioctl failed: ") + ::strerror(errno)});
        }
...
Result<uint64_t, SLError> SpliceEngine::map_into_process(pid_t pid,
    ...
    if (vpu_fd_ >= 0 && phys_addr != 0) {
        ...
        std::string sysfs_path = "/sys/class/straylight-vpu/map_to_pid";
        std::ofstream sysfs(sysfs_path);
        if (sysfs.is_open()) {
            sysfs << pid << " " << target_addr << " " << phys_addr << " " << size << "\n";
            sysfs.flush();
            if (sysfs.good()) {
                ::close(mem_fd);
...
void SpliceEngine::unmap_from_process(pid_t pid, uint64_t addr, uint64_t size) {
    ...
    if (vpu_fd_ >= 0) {
        std::string sysfs_path = "/sys/class/straylight-vpu/unmap_from_pid";
        std::ofstream sysfs(sysfs_path);
        if (sysfs.is_open()) {
            sysfs << pid << " " << addr << " " << size << "\n";
            sysfs.flush();
