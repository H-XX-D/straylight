...
public:
    ...
    Result<void, SLError> tick() override {
        ...
        // In production, we would also poll D-Bus here via sd-bus or dbus-1.
...
int main(int /*argc*/, char* /*argv*/[]) {
    ...
    if (!cfg_result.has_value()) {
        SL_WARN("notify: no config found, using defaults");
        // Create a minimal default config in memory.
        auto default_cfg = straylight::Config::load("/dev/null");
        if (!default_cfg.has_value()) {
            SL_ERROR("notify: cannot create default config");
            return 1;
        }
