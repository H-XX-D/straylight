...
public:
    PredictDaemon() : DaemonBase("straylight-predict") {
        set_tick_interval_ms(1000); // 1-second ticks; predict cycle managed internally
    }
...
protected:
    ...
    void on_reload() override {
        fprintf(stdout, "[straylight-predict] reloading: retrain + evict stale preloads\n");
        engine_.train();
        preloader_.evict_all();
    }
...
private:
    ...
    std::string handle_train(int id) {
        auto result = engine_.train();
        if (!result) {
            return rpc_error(id, -32000, result.err());
        }
