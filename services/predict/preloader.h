    ...
    VoidResult<> preload_app(const std::string& app_name) {
        ...
        if (is_gpu_app(app_name)) {
            ...
            if (vpu_result) {
                PreloadEntry entry;
                entry.app_name = app_name;
                entry.resource_path = "/sys/class/straylight-vpu/vpu0/prealloc";
...
private:
    ...
    Result<size_t, std::string> preload_vpu_slab(const std::string& app_name) {
        // Write to VPU sysfs to pre-allocate a slab
        std::string prealloc_path = "/sys/class/straylight-vpu/vpu0/prealloc";
        std::ofstream f(prealloc_path);
        if (!f) {
            return Result<size_t, std::string>::error("VPU sysfs not available");
        }
        ...
        f << app_name << " " << slab_size;
        if (!f) {
            return Result<size_t, std::string>::error("VPU prealloc write failed");
        }
    ...
    void release_vpu_slab(const std::string& app_name) {
        std::string release_path = "/sys/class/straylight-vpu/vpu0/release";
        std::ofstream f(release_path);
        if (f) {
            f << app_name;
        }
    }
