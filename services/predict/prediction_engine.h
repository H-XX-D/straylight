...
public:
    ...
    Result<int, std::string> train() {
        ...
        if (db_path.empty()) {
            return Result<int, std::string>::error("timeline.db not found");
        }
...
private:
    ...
    std::string get_timeline_db_path() const {
        ...
        if (!home) return "";
        std::string path = std::string(home) + "/.local/share/straylight/timeline.db";
        // Check if file exists via access()
    ...
    Result<int, std::string> train_from_procfs() {
        ...
        std::string output = exec_cmd(
            "ps -eo comm --no-headers 2>/dev/null | sort | uniq -c | sort -rn | head -30");
