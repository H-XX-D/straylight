...
bool PolicyEngine::apply_compositor(const SystemRole& role, ApplyResult& result) {
    if (role.compositor.mode == "disabled") {
        // Stop compositor
        if (run_command("systemctl stop straylight-compositor 2>/dev/null")) {
            result.actions.push_back("Stopped compositor");
            result.changes_made++;
        }
        return true;
    }

    nlohmann::json comp_conf;
    std::ifstream existing("/etc/straylight/compositor.conf");
    if (existing.is_open()) {
        try { comp_conf = nlohmann::json::parse(existing); } catch (...) {}
    }

    comp_conf["mode"] = role.compositor.mode;
    comp_conf["max_fps"] = role.compositor.max_fps;
    comp_conf["vsync"] = role.compositor.vsync;
    comp_conf["direct_scanout"] = role.compositor.direct_scanout;
    comp_conf["low_latency_input"] = role.compositor.low_latency_input;

    if (write_config("/etc/straylight/compositor.conf", comp_conf)) {
        reload_daemon("compositor");
        result.actions.push_back("Set compositor mode to '" + role.compositor.mode +
                                  "' (" + std::to_string(role.compositor.max_fps) + "fps)");
        result.changes_made++;
        return true;
    }

    result.errors.push_back("Failed to update compositor config");
    result.changes_failed++;
    return false;
}
