...
CheckResult HealthChecks::check_network() {
    ...
    if (!gw.empty()) {
        while (!gw.empty() && gw.back() == '\n') gw.pop_back();
        // Try to reach gateway
        auto ping = run_cmd("ping -c 1 -W 2 " + gw + " 2>/dev/null");
    ...
    // DNS
    auto dns = run_cmd("host -W 2 google.com 2>/dev/null || nslookup google.com 2>/dev/null");
    if (dns.find("address") != std::string::npos ||
...
CheckResult HealthChecks::check_services() {
    ...
    for (const auto& svc : expected) {
        ...
        if (!out.empty()) ++running;
    }

    int total = static_cast<int>(expected.size());
    if (total == 0) {
        cr.score = 100;
        cr.status = HealthStatus::Ok;
        cr.detail = "No services to monitor";
    } else {
        cr.score = (running * 100) / total;
        if (running == total) {
            cr.status = HealthStatus::Ok;
            cr.detail = "All " + std::to_string(total) + " services running";
        } else {
            cr.status = (running > 0) ? HealthStatus::Warn : HealthStatus::Critical;
            cr.detail = std::to_string(running) + "/" + std::to_string(total) + " services running";
        }
    }
