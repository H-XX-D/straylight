...
std::vector<AudioDevice> AudioEngine::parse_pw_devices(const std::string& json) const {
    ...
    std::regex class_re(R"("media\.class"\s*:\s*"([^"]*)")");
    std::regex name_re(R"("node\.name"\s*:\s*"([^"]*)")");
    std::regex desc_re(R"("node\.description"\s*:\s*"([^"]*)")");
    std::regex nick_re(R"("node\.nick"\s*:\s*"([^"]*)")");
...
std::vector<AudioStream> AudioEngine::parse_pw_streams(const std::string& json) const {
    ...
    std::regex app_re(R"("application\.name"\s*:\s*"([^"]*)")");
    std::regex bin_re(R"("application\.process\.binary"\s*:\s*"([^"]*)")");
    std::regex pid_re(R"("application\.process\.id"\s*:\s*"(\d+)")");
    std::regex media_re(R"("media\.name"\s*:\s*"([^"]*)")");
