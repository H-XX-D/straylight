        ...
        // Clean up finished threads
        {
            ...
            client_threads_.erase(
                std::remove_if(client_threads_.begin(), client_threads_.end(),
                               [](std::thread& t) {
                                   if (t.joinable()) {
                                       // Can't check if done without platform-specific APIs,
                                       // so we just keep them until stop()
                                       return false;
                                   }
                                   return true;
                               }),
            ...
            client_threads_.emplace_back(&AliceIpcServer::handle_client, this, client_fd);
        }
    }
}
