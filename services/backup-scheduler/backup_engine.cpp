...
Result<BackupRecord, SLError> BackupEngine::run_backup(const std::string& schedule_name) {
    ...
    if (!target) {
        return Result<BackupRecord, SLError>::error(
            SLError{SLErrorCode::NotFound, "No schedules configured"});
    }
...
Result<BackupRecord, SLError> BackupEngine::snapshot(
    ...
    std::string cmd = "tar -cf - " + source + " 2>/dev/null | zstd -3 -o " +
                      dest.string() + " 2>&1";
...
Result<BackupRecord, SLError> BackupEngine::rsync_to_remote(
    ...
    if (!target.ssh_key.empty()) {
        cmd += " -e 'ssh -i " + target.ssh_key + " -p " + std::to_string(target.port) + "'";
    } else if (target.port != 22) {
        cmd += " -e 'ssh -p " + std::to_string(target.port) + "'";
    }
    cmd += " " + source + " " + remote_path + " 2>&1";
...
Result<void, SLError> BackupEngine::restore(const std::string& backup_id,
    ...
    if (rec->encrypted) {
        ...
        std::string cmd = "gpg -d -o " + dec_path + " " + source.string() + " 2>&1";
    ...
    std::string cmd = "zstd -d " + source.string() + " --stdout | tar -xf - -C " + dest + " 2>&1";
...
Result<bool, SLError> BackupEngine::verify(const std::string& backup_id) {
    ...
    std::string cmd;
    if (rec->encrypted) {
        cmd = "gpg -d " + source.string() + " 2>/dev/null | zstd -t 2>&1";
    } else {
        cmd = "zstd -t " + source.string() + " 2>&1";
    }
...
Result<fs::path, SLError> BackupEngine::encrypt_file(
    ...
    encrypted += ".gpg";
    std::string cmd = "gpg --batch --yes -e -r " + recipient +
                      " -o " + encrypted.string() + " " + file.string() + " 2>&1";
    if (exec_return_code(cmd) != 0) {
