...
uint64_t QuotaEngine::read_vram_usage(pid_t pid) {
    // StrayLight VPU exposes per-process VRAM usage via sysfs
    // /sys/devices/virtual/straylight-vpu/vram/proc/<pid>/usage_bytes
    std::string path = "/sys/devices/virtual/straylight-vpu/vram/proc/" +
