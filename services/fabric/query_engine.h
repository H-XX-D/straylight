...
public:
    ...
    Result<TopologyPath, std::string> query(const std::string& query_str) const {
        ...
        if (!from_res) return Result<TopologyPath, std::string>::error(from_res.err());
        auto to_res = resolve_device_ref(to_device);
        if (!to_res) return Result<TopologyPath, std::string>::error(to_res.err());
    ...
    Result<AffinityResult, std::string> get_affinity(const std::string& device_ref) const {
        ...
        if (!dev_res) return Result<AffinityResult, std::string>::error(dev_res.err());

        auto node_res = topo_.get_node(dev_res.value());
        if (!node_res) return Result<AffinityResult, std::string>::error(node_res.err());
    ...
    Result<TransferEstimate, std::string> estimate_transfer_time(
        ...
        if (!from_res) return Result<TransferEstimate, std::string>::error(from_res.err());
        auto to_res = resolve_device_ref(to_ref);
        if (!to_res) return Result<TransferEstimate, std::string>::error(to_res.err());

        auto path_res = topo_.find_fastest_path(from_res.value(), to_res.value());
        if (!path_res) return Result<TransferEstimate, std::string>::error(path_res.err());
    ...
    Result<double, std::string> get_bandwidth(const std::string& from_ref,
        ...
        auto from_res = resolve_device_ref(from_ref);
        if (!from_res) return Result<double, std::string>::error(from_res.err());
        auto to_res = resolve_device_ref(to_ref);
        if (!to_res) return Result<double, std::string>::error(to_res.err());

        auto path_res = topo_.find_fastest_path(from_res.value(), to_res.value());
        if (!path_res) return Result<double, std::string>::error(path_res.err());
