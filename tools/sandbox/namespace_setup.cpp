...
Result<void, std::string> setup_net_namespace(pid_t pid,
    ...
    auto r3 = shell("ip addr add 10.0.0.1/24 dev " + veth_host + " 2>&1");
    if (!r3.has_value()) {
        return Result<void, std::string>::error(
    ...
    auto r8 = shell("nsenter -t " + std::to_string(pid) +
                     " -n ip route add default via 10.0.0.1 2>&1");
    if (!r8.has_value()) {
        // Non-fatal.
    }
    ...
    shell("iptables -t nat -A POSTROUTING -s 10.0.0.0/24 -j MASQUERADE 2>&1");
