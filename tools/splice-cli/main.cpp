...
static void print_usage() {
    std::cerr
        << "straylight-splice -- Zero-copy pipeline stitching CLI\n\n"
        << "Usage:\n"
        << "  straylight-splice create <pid1> <pid2> [--size=N]  Create splice session\n"
        << "  straylight-splice list                              List active sessions\n"
        << "  straylight-splice stats <session-id>                Show session statistics\n"
        << "  straylight-splice destroy <session-id>              Destroy a session\n";
}
...
static int cmd_stats(straylight::IpcJsonClient& client, int argc, char* argv[]) {
    if (argc < 3) {
...
static int cmd_destroy(straylight::IpcJsonClient& client, int argc, char* argv[]) {
    if (argc < 3) {
