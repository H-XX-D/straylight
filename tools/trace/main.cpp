...
int main(int argc, char* argv[]) {
    ...
    if (command == "run" || command == "record") {
        ...
        int i = 2;
        // First, find the command to trace (before any -- or options that follow)
        while (i < argc) {
            ...
            if (arg == "--max-events") {
                if (i + 1 < argc) max_events = static_cast<size_t>(std::atol(argv[++i]));
                ++i;
                continue;
            }
            if (arg == "--no-events") {
        ...
        // Remaining args are the command to trace
        for (; i < argc; ++i) {
