...
struct IntentState {
    ...
    void resolve(const char* text) {
        ...
        if (s.find("restart") != std::string::npos || s.find("service") != std::string::npos) {
            preview.push_back({"Stop the target service", "systemctl stop <service>", 1});
            preview.push_back({"Start the target service", "systemctl start <service>", 0});
            preview.push_back({"Verify service status", "systemctl status <service>", 0});
        } else if (s.find("update") != std::string::npos || s.find("upgrade") != std::string::npos) {
            preview.push_back({"Sync package database", "pacman -Sy", 0});
            preview.push_back({"Create system snapshot", "snapper create --description pre-update", 0});
            preview.push_back({"Upgrade all packages", "pacman -Su --noconfirm", 1});
        } else if (s.find("kill") != std::string::npos || s.find("stop") != std::string::npos) {
            preview.push_back({"Identify target process", "pgrep -f <pattern>", 0});
            preview.push_back({"Send SIGTERM to process", "kill <pid>", 1});
        } else if (s.find("disk") != std::string::npos || s.find("storage") != std::string::npos) {
            preview.push_back({"Scan disk usage", "du -sh /* 2>/dev/null | sort -rh", 0});
            preview.push_back({"Check filesystem health", "btrfs scrub start /", 0});
        } else if (s.find("delete") != std::string::npos || s.find("remove") != std::string::npos) {
            preview.push_back({"Identify target files", "find / -name <pattern>", 0});
            preview.push_back({"Remove matching files", "rm -rf <targets>", 2});
        } else {
            preview.push_back({"Analyze intent", "straylight-intent parse \"" + std::string(text) + "\"", 0});
            preview.push_back({"Execute resolved action", "<resolved command>", 1});
        }
    }
};
