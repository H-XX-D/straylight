...
Result<void, std::string> DisplayPage::apply() {
    ...
    // Signal the compositor to reload display config
    // In production: send via Wayland protocol or D-Bus
    FILE* pipe = popen("pkill -USR1 straylight-compositor 2>/dev/null", "r");
    if (pipe) pclose(pipe);
