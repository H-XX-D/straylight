...
struct ScanState {
    // Scan tab
    char subnet_input[128] = "192.168.1.0/24";
    bool scanning = false;
    float scan_progress = 0.0f;
    std::vector<HostResult> hosts;

    // Port scan tab
    char port_host[128] = "192.168.1.1";
    ...
    void init() {
        // Populate sample scan results
        hosts.push_back({"192.168.1.1", "AA:BB:CC:DD:EE:01", "Ubiquiti", "gateway.local", "22, 80, 443"});
        hosts.push_back({"192.168.1.10", "AA:BB:CC:DD:EE:10", "Dell Inc.", "workstation-01", "22, 3389"});
        hosts.push_back({"192.168.1.20", "AA:BB:CC:DD:EE:20", "NVIDIA", "gpu-node-01", "22, 8080, 9090"});
        hosts.push_back({"192.168.1.21", "AA:BB:CC:DD:EE:21", "NVIDIA", "gpu-node-02", "22, 8080, 9090"});
        hosts.push_back({"192.168.1.30", "AA:BB:CC:DD:EE:30", "Synology", "nas-01.local", "22, 80, 443, 5000, 5001"});
        hosts.push_back({"192.168.1.50", "AA:BB:CC:DD:EE:50", "Raspberry Pi", "pihole.local", "22, 53, 80"});
        hosts.push_back({"192.168.1.100", "AA:BB:CC:DD:EE:A0", "Intel Corp.", "dev-laptop", "22"});
        hosts.push_back({"192.168.1.200", "AA:BB:CC:DD:EE:C8", "Apple Inc.", "macbook-pro", "22, 5900"});

        port_results.push_back({22, "ssh"});
        port_results.push_back({80, "http"});
        port_results.push_back({443, "https"});
        port_results.push_back({8080, "http-alt"});
        port_results.push_back({9090, "prometheus"});

        trace_hops.push_back({1, "192.168.1.1", "gateway.local", 0.8f, false});
        trace_hops.push_back({2, "10.0.0.1", "isp-pe-1.net", 5.2f, false});
        trace_hops.push_back({3, "172.16.0.1", "core-rtr.isp.net", 12.4f, false});
        trace_hops.push_back({4, "0.0.0.0", "*", 0.0f, true});
        trace_hops.push_back({5, "108.170.250.33", "google-edge.net", 18.7f, false});
        trace_hops.push_back({6, "8.8.8.8", "dns.google", 20.1f, false});

        bw_download = 940.5f;
        bw_upload = 520.3f;
        for (int i = 0; i < 120; ++i) {
            bw_history[i] = 800.0f + 200.0f * sinf(i * 0.1f) + (float)(i % 7) * 15.0f;
        }

        health_indicators.push_back({"Gateway Reachable", 0, "192.168.1.1 responds in < 1ms"});
        health_indicators.push_back({"DNS Resolution", 0, "Resolving names in < 5ms"});
        health_indicators.push_back({"Internet Connectivity", 0, "External hosts reachable"});
        health_indicators.push_back({"Packet Loss", 0, "0.0% packet loss"});
        health_indicators.push_back({"Latency", 1, "Average 22ms (slightly elevated)"});
        health_indicators.push_back({"Bandwidth", 0, "940 Mbps down / 520 Mbps up"});
        health_indicators.push_back({"ARP Table", 0, "8 entries, no conflicts"});
        health_indicators.push_back({"DHCP Lease", 0, "Valid, 23h remaining"});
...
inline void render_scan_panel(ScanState& st) {
    ...
    if (ImGui::BeginTabBar("##probe_tabs")) {
        ...
        if (ImGui::BeginTabItem("Network Scan")) {
            ...
            ImGui::InputTextWithHint("##subnet", "Subnet (e.g. 192.168.1.0/24)", st.subnet_input, sizeof(st.subnet_input));
            ImGui::SameLine();
            if (st.scanning) {
                ImGui::BeginDisabled();
        ...
        if (ImGui::BeginTabItem("Bandwidth")) {
            ...
            // Animate
            st.bw_history[st.bw_history_offset] = st.bw_download + (float)((int)(ImGui::GetTime() * 100) % 50) - 25.0f;
            st.bw_history_offset = (st.bw_history_offset + 1) % 120;
